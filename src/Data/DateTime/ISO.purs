module Data.DateTime.ISO (ISO(..), unwrapISO, parseISO) where
...
instance decodeJsonISO :: Decode ISO where
  decode f = do
    s :: String <- decode f
    except $ lmap (pure <<< ForeignError <<< P.parseErrorMessage) $ P.runParser s parseISO

instance encodeJsonISO :: Encode ISO where
  encode = show >>> encode
